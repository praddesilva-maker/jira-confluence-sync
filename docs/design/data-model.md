...
types and KVS key layout (`docs/design/solution-architecture.md` §10), kept in sync with the code
in the same PR that changes either.

## Types (as built)

_None yet — Phase 0 has no domain layer._

## KVS key layout (as built)

_None yet — Phase 0 declares `storage:app` scope but writes nothing to KVS._
